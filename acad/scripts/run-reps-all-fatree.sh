...
for TXT_WORKLOAD in ${TXT_WORKLOADS[@]};do
	...
	for ALG in ${ALGS[@]};do
		...
		for ALLREDUCE_ALG in ${ALLREDUCE_ALGS[@]};do
			...
			WORKLOAD=${ET_WORKLOAD_DIR}/$TXT_WORKLOAD-$K-fat-tree
			SYSTEM=${SYSTEM_DIR}/system-$ALLREDUCE_ALG-$APP_LOADBALANCE_ALG.json
			NETWORK=${NETWORK_DIR}/config-fat-tree-$K-${ROUTING}-${APP_LOADBALANCE_ALG}-${ALLREDUCE_ALG}-${TXT_WORKLOAD}.txt
			MEMORY=${MEMORY_DIR}/remote_memory.json
			LOGICAL_TOPOLOGY=${LOGICAL_TOPO_DIR}/logical-topo-$NUM_NODES.json
			OUTPUT_FILE=${RESULTS_DIR}/AllReduce-$NUM_NODES-$TXT_WORKLOAD-fat-tree-$ALG-$ALLREDUCE_ALG.out
