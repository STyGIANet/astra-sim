...
void qp_finish_print_log(FILE *fout, Ptr<RdmaQueuePair> q) {
  ...
  fprintf(fout, "%08x %08x %u %u %lu %lu %lu %lu %lu %lu %lu %lu\n", q->sip.Get(), q->dip.Get(),
          q->sport, q->dport, q->m_size, q->startTime.GetNanoSeconds(),
          (Simulator::Now() - q->startTime).GetNanoSeconds(), standalone_fct, Simulator::Now().GetNanoSeconds(), q->m_src, q->m_dest, q->maxQps);
  fflush(fout);
}
